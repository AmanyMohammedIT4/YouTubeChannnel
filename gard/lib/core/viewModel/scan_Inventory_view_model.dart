...
import 'package:gard/core/services/database/scan_inventory_database.dart';
import 'package:gard/model/items_model.dart';
import 'package:get/get.dart';
import 'package:internet_connection_checker/internet_connection_checker.dart';

class ScanInventoryViewModel extends GetxController{
...
List<int> get currentQuantities=> _currentQuantities;
...
ScanInventoryDatabaseHelper scanInventoryDatabaseHelper =ScanInventoryDatabaseHelper();
final conn = ScanInventoryDatabaseHelper.instance;
...
Future<List<Map<dynamic, dynamic>>> getDataProduct() async {
  try {
    productNameList = []; // تهيئة القائمة لتكون فارغة
    ...
    int productCounter = 1; // متغير يتم زيادته بشكل تلقائي لتعيين رقم ثابت لكل منتج

    List<Map<dynamic, dynamic>> result = querySnapshot.docs.map((doc) => doc.data()).toList();

    querySnapshot.docs.forEach((doc) {
      String productName = doc['name'];
      productNameList.add('$productCounter.$productName'); // تعيين رقم ثابت لكل منتج
      productCounter++; // زيادة العداد للحصول على رقم ثابت للمنتج التالي
    });
...
void addToFirestore(String email, String nameInv,String barcodeLists,
String productName,DateTime date,int count) async {
   for(int i=0 ; i< barcodeList.length; i++){
   try {
        print('barcodeList.length in scan view model: ${barcodeList.length}');
        
        final CollectionReference collection = FirebaseFirestore.instance.collection('scanBarcodeInventory');
...
Future<void> saveToFirestoreRefresh(List<Map<String, dynamic>> data) async {
  ...
  CollectionReference collection =
      FirebaseFirestore.instance.collection('scanBarcodeInventory');

  for (var record in data) {
    await collection.add(record);
    scanInventoryDatabaseHelper.deleteTable(tableScanInventory);
     update();
  }
}
...
void removeFromBarcodeList(String barcode) {
  ...
  _currentQuantities[index] = newQuantity;
  update();
}
